...
interface BorderConfigSansTitled {
    ...
    fun line(
        ...
        init: ColorConfig.() -> Unit,
    )

    // Bevel borders
    fun bevel(init: BevelConfig.() -> Unit)
    fun softBevel(init: BevelConfig.() -> Unit) = bevel { soft(); init() }
    fun loweredBevel(init: BevelConfig.() -> Unit = {}) =
        bevel { lowered(); init() }

    fun raisedBevel(init: BevelConfig.() -> Unit = {}) =
        bevel { raised(); init() }
    ...
    fun loweredSoftBevel(init: BevelConfig.() -> Unit = {}) =
        softBevel { lowered(); init() }

    fun raisedSoftBevel(init: BevelConfig.() -> Unit = {}) =
        softBevel { raised(); init() }
    ...
    fun loweredEtched(init: EtchedConfig.() -> Unit = {}) =
        etched { lowered(); init() }

    fun raisedEtched(init: EtchedConfig.() -> Unit = {}) =
        etched { raised(); init() }
