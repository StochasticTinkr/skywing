...
class LinearGradientBuilder(val start: Point2D, val end: Point2D) {
    ...
    fun transform(init: AffineTransform.() -> Unit) = transform.init()
...
inline fun linearGradient(start: Point2D, end: Point2D, init: LinearGradientBuilder.() -> Unit) =
    LinearGradientBuilder(start, end).apply(init).build()